...
module.exports = class RevaneFastify {
  ...
  register (plugin, options) {
    this.promise = this.promise.then(() => {
      if (typeof plugin === 'string') {
        ...
        if (isBindable(pluginById.plugin)) {
          pluginById.plugin = pluginById.plugin.bind(pluginById)
        }
        if (!isPlugin(pluginById.plugin)) {
          pluginById.plugin = fastifyPlugin(pluginById.plugin)
        }
  ...
  listen (addressProviderId) {
    return this.promise
      .then(() => {
        const { host, port } = this._getHostAndPort(addressProviderId)
        ...
        return new Promise((resolve, reject) => {
          this.server.listen(port, host, (err, address) => {
            if (err) {
              ...
              this._logStartUp()
              resolve({ address, revane: this.revane })
            }
          })
        })
      })
    ...
    this.promise = this.promise
      .then(() => {
        if (typeof handler === 'string') {
          const errorHandler = this.revane.get(handler)
  ...
  setNotFoundHandler (handler) {
    this.promise = this.promise
      .then(() => {
        if (typeof handler === 'string') {
          const notFoundHandler = this.revane.get(handler)
  ...
  _getHostAndPort (addressProviderId) {
    let host
    let port
    if (typeof addressProviderId === 'string') {
      const addressProvider = this.revane.get(addressProviderId)
