...
test('should be able call @All handler with POST', async (t) => {
  ...
  instance.unref()
  const port = instance.port()
  const response = await fetch(`http://localhost:${port}/something`, { method: 'POST' })
  const data = await response.text()
  t.is(response.status, 200)
  t.is(data.toString(), 'hello world')
  instance.close()
...
test('should pass cookie value to handler', async (t) => {
  ...
  instance.unref()
  const port = instance.port()
  const response = await fetch(`http://localhost:${port}/something`, { headers: { cookie: 'test=hello world; Path=/; HttpOnly' }})
  const data = await response.text()
  t.is(response.status, 200)
  t.is(data.toString(), 'hello world')
  instance.close()
...
test('should pass cookie values to handler', async (t) => {
  ...
  instance.unref()
  const port = instance.port()
  const response = await fetch(`http://localhost:${port}/cookies`, { headers: { cookie: 'test=hello world; Path=/; HttpOnly' }})
  const data = await response.text()
  t.is(response.status, 200)
...
test('should pass header value to handler with alternate name', async (t) => {
  ...
  instance.unref()
  const port = instance.port()
  const response = await fetch(`http://localhost:${port}/header`, { headers: { 'x-test': 'test' }})
  const data = await response.text()
  t.is(response.status, 200)
...
test('should pass header values to handler', async (t) => {
  ...
  instance.unref()
  const port = instance.port()
  const response = await fetch(`http://localhost:${port}/headers`, { headers: { 'x-test': 'test' }})
  const data = await response.text()
  t.is(response.status, 200)
  t.is(data.toString(), 'true')
  instance.close()
...
test('should pass request body to handler', async (t) => {
  ...
  const response = await fetch(
    `http://localhost:${port}/requestpost`,
    {
      method: 'POST',
